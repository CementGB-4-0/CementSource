    ...
    private static class CostumeDatabasePatch
    {
        private static void Postfix(bool __result, CostumeDatabase._Load_d__12 __instance)
        {
            ...
            foreach (var location in AssetUtilities.GetAllModdedResourceLocationsOfType<CostumeObject>())
            {
                ...
                {
                    Mod.Logger.Error(
                        $"Failed to load custom Addressable CostumeObject : Key \"{location.PrimaryKey}\" : OperationException {handle.OperationException?.ToString() ?? "null"}");
                    handle.Release();
                    continue;
                }
                ...
                {
                    Mod.Logger.Error(
                        $"Handle loading Custom CostumeObject completed with no result : Key \"{location.PrimaryKey}\" : OperationException {handle.OperationException?.ToString() ?? "null"}");
                    handle.Release();
                    continue;
                }
                ...
                Mod.Logger.Msg(ConsoleColor.DarkGreen,
                    $"New custom CostumeObject registered : Key \"{location.PrimaryKey}\"");
            }

            timeTakenStopwatch.Stop();
            Mod.Logger.Msg(ConsoleColor.Green,
                $"Custom CostumeObject injection completed! Total time taken: {timeTakenStopwatch.Elapsed}");
        }
    }
}