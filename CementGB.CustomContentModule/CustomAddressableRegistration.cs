...
public static class CustomAddressableRegistration
{
    ...
    internal static string ResolveModdedInternalId(string bundleFile)
    {
                     ...
                     SearchOption.AllDirectories))
        {
            ...
            {
                Logger?.VerboseLog($"Skipping catalog path {catalogPath}. . .");
                continue;
            }
            ...
            Logger?.VerboseLog(ConsoleColor.DarkGreen, $"Resolved InternalId: {result}. . .");
            return result;
        }
    ...
    {
        Logger?.Msg("Caching base game Addressable keys. . .");
    ...
    private static void InitializeContentCatalogs()
    {
        ...
        Logger?.Msg("Starting initialization of modded Addressable content catalogs. . .");
                     ...
                     SearchOption.AllDirectories))
        {
            ...
            foreach (var file in Directory.EnumerateFiles(aaPath, "catalog_*.json", SearchOption.AllDirectories))
            {
                ...
                Logger?.Msg(ConsoleColor.Green, $"Content catalog for \"{addressablePackName}\" loaded OK");
            }
        }

        stopwatch.Stop();
        Logger?.Msg(ConsoleColor.Green, $"Done custom content catalogs! Total time taken: {stopwatch.Elapsed}");
    }

    private static void InitializeMapReferences()
    {
        Logger?.Msg("Starting initialization of custom map references. . .");
        var stopwatch = new Stopwatch();
        stopwatch.Start();
